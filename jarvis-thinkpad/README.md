...
| Voice engine | Kokoro `bm_lewis` for English, Piper `bg_BG-dimitar-medium` for Bulgarian (both free, local); ElevenLabs off | Decided: zero cost, offline, two voices, one per language. ElevenLabs would give one voice for both at $22 to $99 a month; it stays off unless you say "use the premium voice" (key in `~/.config/flint/elevenlabs.env`, never in a file he reads) |
