...
run() {
  ...
  if has obsidian; then ok "already installed"; else
    ...
    tmp="$(mktemp -d)"
    if [ -n "$deb" ]; then
      curl -fL --retry 3 "$deb" -o "$tmp/obsidian.deb" || die "download failed: $deb"
      aptget install -y -qq "$tmp/obsidian.deb"
      ...
      curl -fL --retry 3 "$app" -o "$HOME/.local/bin/obsidian" || die "download failed: $app"
  ...
  if has google-chrome; then ok "already installed"
    ...
    curl -fL --retry 3 https://dl.google.com/linux/direct/google-chrome-stable_current_amd64.deb -o "$tmp/chrome.deb" || die "Chrome download failed"
    aptget install -y -qq "$tmp/chrome.deb"; rm -rf "$tmp"
  ...
  if [ "$CLAUDE_DESKTOP" = 1 ]; then
    ...
    if has claude-desktop; then ok "already installed"; else
      if sudo curl -fsSLo /usr/share/keyrings/claude-desktop-archive-keyring.asc https://downloads.claude.ai/claude-desktop/key.asc \
         && gpg --show-keys /usr/share/keyrings/claude-desktop-archive-keyring.asc 2>/dev/null | grep -q 31DDDE24DDFAB679F42D7BD2BAA929FF1A7ECACE; then
      ...
      else warn "Claude Desktop signing key not downloadable or not the documented one; optional, skipped"; fi
    fi
  fi

  if [ "$VSCODE" = 1 ]; then
    log "VS Code"
    has code || { sudo snap install code --classic >/dev/null 2>&1 && ok "installed (snap)" || warn "VS Code snap not installable right now; optional, skipped"; }
  fi
}

check() {
  chk "obsidian" has obsidian
  chk_warn "obsidian not running (the vault stage closes it if it must)" bash -c '! pgrep -x obsidian >/dev/null'
