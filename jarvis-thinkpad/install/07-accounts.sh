...
# the probe runs from a folder without a CLAUDE.md, so one turn is one answer
claude_ok() { claude auth status >/dev/null 2>&1 || (cd "$STATE_DIR" && timeout 90 claude -p "reply with exactly: ok" --max-turns 1 --output-format text </dev/null 2>/dev/null | grep -qi 'ok'); }
skip_now() { [ "$ACCOUNTS_LATER" = 1 ] && [ "${FLINT_ONLY:-}" != 07 ]; }

run() {
  if skip_now; then warn "ACCOUNTS_LATER=1: skipping the logins. Later: setup.sh --only 07"; return 0; fi
  [ -t 0 ] && [ -t 1 ] || die "the two logins need a real terminal. Open one and run: setup.sh --only 07"
  ...
  if claude_ok; then ok "already logged in"; else
    ...
    claude_ok || { warn "not logged in yet; one more try"; claude auth login || true; }
...
check() {
  if skip_now; then warn "skipped by ACCOUNTS_LATER=1 (setup.sh --only 07 runs the logins anyway)"; return 0; fi
