...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --from) FROM="$2"; shift ;;
    --only) ONLY="$2"; shift ;;
    --reset) rm -rf "$DONE" "$STATE/reboot-needed" "$STATE/.apt-updated"; mkdir -p "$DONE"; echo "progress forgotten"; exit 0 ;;
    --continue) : ;;
...
for s in "${STAGES[@]}"; do
  ...
  logf="$LOGS/${s%.sh}.log"
  if [ "$MODE" = check ]; then
    bash "$INSTALL/$s" check 2>&1 | tee -a "$logf" || FAILED=$((FAILED+1))
    continue
  fi
  if bash "$INSTALL/$s" run 2>&1 | tee -a "$logf" && bash "$INSTALL/$s" check 2>&1 | tee -a "$logf"; then
    touch "$DONE/$s"
  else
